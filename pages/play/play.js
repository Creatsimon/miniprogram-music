...
Page({
    ...
    //上一首
    playLast(){
        let palyList = this.data.playList
        for(let i = 0 ; i < palyList.length ; i++){
            if(this.data.songId == palyList[i].id){
                this.changeSong2(palyList[i-1].id)
                break;
            }
        }
    },
    ...
    //点击播放列里的歌曲
    changeSong(e){
        this.loadSongInfo(e.currentTarget.dataset.songid)
        this.loadSong(e.currentTarget.dataset.songid)
        this.loadLyrics(e.currentTarget.dataset.songid)
        this.resetTop()
        this.setData({
    ...
    loadSingerInfo(id) {
        ...
        //         success: (res) => {
        //             let info = {
        //                 id:res.data.data.artist.id,
        //                 name:res.data.data.artist.name,
        //                 avatar:res.data.data.artist.avatar
        //             }
        //             _this.data.singerInfos.info.push(info)
        //         }
        //     })
        // }
        ...
        wx.request({
            url: 'http://' + ip + ':3000/artist/detail?id=' + id,
            dataType: 'json',
            success: (res) => {
                let info = {
                    id:res.data.data.artist.id,
                ...
                this.data.singerInfos.info.push(info)
            }
        })
    ...
    openSingerOrAlbum() {
        ...
        //  for(let i = 0; i < this.data.songDetail[0].ar.length; i++){
        //     this.loadSingerInfo(this.data.songDetail[0].ar[i].id)
        // }
        ...
        this.setData({
            songOrAlbum: true,
            singerInfos:this.data.singerInfos
        })
                ...
                for (let i = 0; i < len; i++) {
                    ...
                    */
                    ...
                    new_arr.forEach((element) => {
                        ...
                        let s2 = parseInt(time_arr[0]) * 60 + Number(time_arr[1]);
            ...
            success: (res) => {
               ...
               var songSingers = []
                for(let i = 0; i < res.data.songs[0].ar.length; i++){
                    wx.request({
                        url: 'http://' + ip + ':3000/artist/detail?id=' + res.data.songs[0].ar[i].id,
                        dataType: 'json',
                        success: (res) => {
                            let info = {
                                id:res.data.data.artist.id,
                            ...
                            songSingers.push(info)
                        }
                    })
                    //this.loadSingerInfo(res.data.songs[0].ar[i].id)
                }
    ...
    onLoad(options) {
        ...
        innerAudioContext.onEnded(() => {
            ...
            for(let i = 0 ; i < playList.length ; i++){
                if(playList[i].id == this.data.songId&&i!=playList.length-1){
                    this.changeSong2(playList[i+1].id)
                    break;
                }
            }
        ...
        innerAudioContext.onTimeUpdate(() => {
            ...
            for (let i = 1; i <= this.data.lyricsInfo.length; i++) {
                if (i == this.data.lyricsInfo.length) {
                    ...
                    if (this.data.lyricsInfo[i].playTime >= innerAudioContext.currentTime) {
                        this.setData({
                            index: i - 1,
                            playIndex: i - 1
                        })
